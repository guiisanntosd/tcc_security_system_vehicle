...
class Detector:
    ...
    def onVideo(self):
        ...
        while ret:
            ...
            classIds, confs, bbox = self.net.detect(frame, confThreshold=0.55)

            bboxs = list(bbox)
            confidences = list(np.array(confs).reshape(1, -1)[0])
            confidences = list(map(float, confidences))

            indices = cv2.dnn.NMSBoxes(bboxs, confidences, score_threshold=0.55, nms_threshold=0.2)

            if len(indices) != 0:
                for i in range(0, len(indices)):
                    if classIds[0] in self.objects:
                        self.sound()
